...
def calc(list_lmbda0, list_alpha, grid_B_phi, grid_B_theta, grid_P_ohm, grid_U_mag, config):
	...
	a = config["a"]
	R0 = aspect_ratio * a
	R_star = 0.969 * R0 # FIXME: use config file
	zneo = config["zneo"]
	zeff = config["zeff"]
	...
	# 	alpha = np.interp(t, loadVec(config_alpha["time"]), loadVec(config_alpha["value"]))
...
def run():
	...
	# Save results
	if "save" in config:
		data = np.column_stack((t, V_phi, V_theta, lmbda0, flux))
		np.savetxt(config["save"].format(config[config["mode"]]["shot"]), data)
